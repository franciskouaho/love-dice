...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    // Afficher automatiquement la modal des noms si pas encore configurés
    setTimeout(() => {
      if (!playerNames.player1.trim() || !playerNames.player2.trim()) {
        console.log('🎭 Showing names modal automatically on startup');
        setIsNamesModalVisible(true);
      }
    }, 1000); // Petit délai pour que l'app soit complètement chargée
  ...
  const { isShaking } = useShake({
    ...
    onShake: async () => {
      ...
      // Vérifier si les noms sont configurés, sinon afficher la modal
      if (!playerNames.player1.trim() || !playerNames.player2.trim()) {
        console.log('🎭 Names not configured, showing modal...');
        setIsNamesModalVisible(true);
        await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
        return;
      }
      
      // Déclencher le lancement des dés
      console.log('🎯 Calling handleRoll from shake...');
      handleRoll();
    }
  });
      ...
      Animated.sequence([
        ...
        if (allFaces.length === 0) {
          ...
          facesToUse = [
            ...
            { id: 'default-payer-2', label: 'Ton copain/ta copine paie', emoji: '💝', category: 'payer', weight: 1 },
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.bottomControls}>
        ...
        {/* Bouton compteur de lancers à droite (déplacé du haut) */}
      ...
      {/* Bouton pour modifier les noms */}
      <View style={styles.nameButtonContainer}>
        <TouchableOpacity 
          style={styles.nameButton}
          onPress={() => setIsNamesModalVisible(true)}
        >
          <View style={styles.nameButtonBlur}>
            <Ionicons name="people" size={18} color="#FFFFFF" />
            <Text style={styles.nameButtonText}>Noms</Text>
          </View>
        </TouchableOpacity>
      </View>
