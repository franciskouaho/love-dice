...
export default function HomeScreen() {
  ...
  const { isShaking } = useShake({
    ...
    onShake: async () => {
      ...
      // Si les noms ne sont pas configurés, utiliser des noms par défaut
      let namesToUse = playerNames;
      if (!playerNames.player1.trim() || !playerNames.player2.trim()) {
        console.log('🎭 Names not configured, using defaults...');
        namesToUse = { player1: "Mon cœur", player2: "Mon amour" };
      }
  ...
  const performRollWithNames = async (namesToUse: { player1: string; player2: string }) => {
    try {
      ...
      Animated.sequence([
        ...
        await saveLastRoll(completeResult);
  ...
  const handleSkipNames = async () => {
    ...
    setIsNamesModalVisible(false);
    await Haptics.selectionAsync();
    performRollWithNames({ player1: "Mon cœur", player2: "Mon amour" });
  };
