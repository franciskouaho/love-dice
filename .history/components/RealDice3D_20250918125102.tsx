...
export const RealDice3D: React.FC<RealDice3DProps> = ({ 
  ...
  // Positions centrées par rapport au centre de l'écran
  const centerX = width / 2;
  const centerY = 350; // Centre vertical approximatif
  const spacing = 100; // Espacement entre les dés
  
  const positions = [
    { left: centerX - spacing, top: centerY - 50 },      // Dé 1 : gauche
    { left: centerX + 20, top: centerY - 50 },           // Dé 2 : droite  
    { left: centerX - 40, top: centerY + 80 },           // Dé 3 : bas centre
  ];
  ...
  return (
    <View style={[styles.container, { backgroundColor: 'rgba(0,255,0,0.1)' }]}>
      <Text style={{ color: 'white', position: 'absolute', top: 10, left: 10, zIndex: 1000, fontSize: 16, fontWeight: 'bold' }}>
        🎲 Dés: {diceData.length} - Width: {Math.round(width)}
      </Text>
