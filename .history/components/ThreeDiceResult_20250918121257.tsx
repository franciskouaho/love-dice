  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.glassContainer}>
        {/* Dés flottants */}
        <View style={styles.floatingDiceContainer}>
          {diceData.map((dice, index) => (
            <View 
              key={`${result.id}-${index}`} 
              style={[
                styles.floatingDice,
                // Positions différentes pour chaque dé
                index === 0 && { top: 20, left: 10 },
                index === 1 && { top: 50, right: 15 },
                index === 2 && { bottom: 30, left: '50%', marginLeft: -DICE_SIZE/2 },
              ]}
            >
              <AnimatedDice3D
                emoji={dice.emoji}
