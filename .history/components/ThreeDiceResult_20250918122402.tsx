...
export const ThreeDiceResult: React.FC<ThreeDiceResultProps> = ({ 
  ...
  // Générer des positions aléatoires mais sécurisées
  const getRandomPositions = () => {
    const safeWidth = width - (insets.left + insets.right + DICE_SIZE + 40); // 40px de marge
    const safeHeight = width * 0.6; // Zone centrale
    const centerX = width / 2;
    const centerY = width * 0.4;
    
    return [
      {
        top: centerY + (Math.random() - 0.5) * safeHeight * 0.6,
        left: centerX + (Math.random() - 0.5) * safeWidth * 0.8,
      },
      {
        top: centerY + (Math.random() - 0.5) * safeHeight * 0.6,
        left: centerX + (Math.random() - 0.5) * safeWidth * 0.8,
  ...
  const randomPositions = useRef(getRandomPositions()).current;
    ...
    <View style={styles.container}>
      ...
      {diceData.map((dice, index) => (
        <View 
          ...
          style={[
            styles.floatingDice,
            // Positions différentes pour chaque dé avec SafeArea
            index === 0 && { top: insets.top + 40, left: insets.left + 40 },
            index === 1 && { top: insets.top + 30, right: insets.right + 40 },
            index === 2 && { bottom: insets.bottom + 50, left: '50%', marginLeft: -DICE_SIZE/2 },
          ]}
        >
          <AnimatedDice3D
            emoji={dice.emoji}
