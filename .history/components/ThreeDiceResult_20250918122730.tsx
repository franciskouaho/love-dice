...
export const ThreeDiceResult: React.FC<ThreeDiceResultProps> = ({ 
  ...
  // Positions aléatoires centrées et clampées dans le conteneur
  const randomPositions = useMemo(() => {
    ...
    const centerX = (containerWidth - DICE_SIZE) / 2;
    const centerY = (containerHeight - DICE_SIZE) / 2;

    // Étendue autour du centre (50% de la zone disponible)
    const spanX = Math.max(0, (maxLeft - minLeft) * 0.5);
    const spanY = Math.max(0, (maxTop - minTop) * 0.5);

    const makePos = () => ({
      left: clamp(centerX + (Math.random() - 0.5) * 2 * spanX, minLeft, maxLeft),
      top: clamp(centerY + (Math.random() - 0.5) * 2 * spanY, minTop, maxTop),
    });

    return [makePos(), makePos(), makePos()];
