...
export const ThreeDiceResult: React.FC<ThreeDiceResultProps> = ({
  ...
  // Positions fixes espacées pour éviter la superposition
  const randomPositions = useMemo(() => {
    const containerWidth = containerSize.w;
    const containerHeight = containerSize.h || width * 0.8;
    
    // Trois colonnes avec espacement garanti
    const margin = 20;
    const usableWidth = containerWidth - (2 * margin) - diceSize;
    const spacing = Math.max(diceSize + 20, usableWidth / 2); // Espacement minimum garanti
    
    const centerY = (containerHeight / 2) - (diceSize / 2);
    const startX = margin;
    
    return [
      { 
        left: startX,
        top: centerY + (Math.random() - 0.5) * 40 // Petite variation verticale
      },
      { 
        left: startX + spacing,
      ...
      { 
        left: startX + spacing * 2,
        top: centerY + (Math.random() - 0.5) * 40
      }
    ].map(pos => ({
      left: Math.max(margin, Math.min(pos.left, containerWidth - diceSize - margin)),
      top: Math.max(insets.top + 20, Math.min(pos.top, containerHeight - diceSize - insets.bottom - 20))
    }));
  }, [containerSize.w, containerSize.h, insets.top, insets.bottom, diceSize, width]);
