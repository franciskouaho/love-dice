...
export const ThreeDiceResult: React.FC<ThreeDiceResultProps> = ({
  ...
  // Disposition en triangle : 1 en haut, 2 en bas
  const randomPositions = useMemo(() => {
    ...
    const topY = insets.top + 80;
    const bottomY = containerHeight - diceSize - 120;
    
    // Espacement horizontal pour les dés du bas
    const spacing = Math.min(120, (containerWidth - 2 * margin - 2 * diceSize) / 2);
    
    const positions = [
      {
        // Dé 1 : en haut au centre
        left: centerX - diceSize / 2,
        top: topY
      },
      {
        // Dé 2 : en bas à gauche
        left: centerX - spacing - diceSize,
        top: bottomY
      },
      {
        // Dé 3 : en bas à droite
        left: centerX + spacing,
        top: bottomY
      }
    ];
    
    // Vérifier que tous les dés restent dans le cadre
    return positions.map(pos => ({
      left: Math.max(margin, Math.min(pos.left, containerWidth - diceSize - margin)),
      top: Math.max(insets.top + 60, Math.min(pos.top, containerHeight - diceSize - 60))
    }));
