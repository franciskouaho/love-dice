...
export const ThreeDiceResult: React.FC<ThreeDiceResultProps> = ({
  ...
  // Positions aléatoires sur tout l'écran avec anti-superposition
  const randomPositions = useMemo(() => {
    ...
    const containerHeight = containerSize.h || width * 0.8;
     ...
     const minTop = insets.top + 60; // Éviter le notch
     const maxTop = containerHeight - diceSize - 100; // Éviter le bas
     
     const positions: { left: number; top: number }[] = [];
     const minDistance = diceSize + 30; // Distance minimum réduite pour plus de possibilités
    
    // Générer 3 positions aléatoires non superposées
    for (let i = 0; i < 3; i++) {
      let attempts = 0;
      let position;
      ...
      do {
        ...
        attempts++;
         ...
         // Si on n'arrive pas après 50 tentatives, forcer une position vraiment aléatoire
         if (attempts > 50) {
           position = {
             left: minLeft + Math.random() * Math.max(0, maxLeft - minLeft),
             top: minTop + Math.random() * Math.max(0, maxTop - minTop)
           };
           break; // On accepte cette position même si elle se superpose un peu
         }
      } while (
