...
export const ThreeDiceResult: React.FC<ThreeDiceResultProps> = ({
  ...
  // Positions aléatoires garanties sans superposition
  const randomPositions = useMemo(() => {
    ...
    const maxLeft = containerWidth - diceSize - margin;
    const minTop = insets.top + 60;
    const maxTop = containerHeight - diceSize - 100;
    
    // Diviser l'écran en 9 zones (3x3) pour garantir l'espacement
    const zones = [];
    const zonesX = 3;
    const zonesY = 3;
    const zoneWidth = (maxLeft - minLeft) / zonesX;
    const zoneHeight = (maxTop - minTop) / zonesY;
    
    // Créer toutes les zones possibles
    for (let x = 0; x < zonesX; x++) {
      for (let y = 0; y < zonesY; y++) {
        zones.push({
          minX: minLeft + x * zoneWidth,
          maxX: minLeft + (x + 1) * zoneWidth - diceSize,
          minY: minTop + y * zoneHeight,
          maxY: minTop + (y + 1) * zoneHeight - diceSize,
        });
      }
    }
    
    // Mélanger les zones et prendre les 3 premières
    const shuffledZones = zones.sort(() => Math.random() - 0.5).slice(0, 3);
    
    // Positionner aléatoirement dans chaque zone
    return shuffledZones.map(zone => ({
      left: Math.max(zone.minX, zone.minX + Math.random() * Math.max(0, zone.maxX - zone.minX)),
      top: Math.max(zone.minY, zone.minY + Math.random() * Math.max(0, zone.maxY - zone.minY))
    }));
