...
export const getDefaultFaces = async (): Promise<DiceFace[]> => {
  try {
    ...
    console.log("📄 Documents trouvés dans Firebase:", querySnapshot.size);

    const faces: DiceFace[] = [];
    querySnapshot.forEach((doc) => {
      const data = doc.data() as FirebaseDiceFace;
      console.log("📝 Face trouvée:", doc.id, data.label, data.category);
    ...
    console.log("✅ Faces par défaut chargées depuis Firebase:", faces.length);
    return faces;
  } catch (error) {
    console.error(
