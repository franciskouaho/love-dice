...
export const rollCompleteDice = (
  ...
  if (playerNames && playerNames.player1.trim() && playerNames.player2.trim()) {
    ...
    console.log('✨ Personalized payer label:', payer.label);
    
    // Forcer la personnalisation si le label n'a pas changé et que c'est "Tu paies"
    if (payer.label === originalLabel && (originalLabel === "Tu paies" || originalLabel === "Je paie")) {
      payer.label = Math.random() < 0.5 ? `${playerNames.player1} paie` : `${playerNames.player2} paie`;
      console.log('🔄 Forced personalization:', payer.label);
    }
  } else {
