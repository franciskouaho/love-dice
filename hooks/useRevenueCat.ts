...
function useRevenueCat() {
    ...
    const hasLifetime = customerInfo?.entitlements?.active?.[LIFETIME_PRODUCT_ID]?.isActive || false;
    ...
    const purchaseLifetime = async () => {
        try {
            ...
            const purchaseResult = await Purchases.purchasePackage(currentOffering.lifetime);

            if (purchaseResult.customerInfo.entitlements.active[LIFETIME_PRODUCT_ID]?.isActive) {
    ...
    const restorePurchases = async () => {
        try {
            ...
            const hasLifetimeAfterRestore = customerInfo.entitlements?.active?.[LIFETIME_PRODUCT_ID]?.isActive || false;
